...
class ShardedSingleStepDataset(ShardedDataset):
    ...
    def __init__(
        ...
        # When True, decode only the video frames each shard actually uses
        # (instead of every frame in the touched episodes). Cuts decode work and
        # memory ~1/episode_sampling_rate with identical data.
    ...
    def get_shard(self, idx: int) -> list:
        ...
        for ep_idx, step_indices in episodes:
            if self.decode_only_used_frames:
                # Only decode the video frames this shard reads from this episode.
                # This MUST mirror extract_step_data's video indexing exactly:
                #   indices = step + video_delta_indices, clamped to [0, len-1]
                #   iff allow_padding (otherwise out-of-range raises in both paths).
