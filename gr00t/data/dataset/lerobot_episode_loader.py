...
class LeRobotEpisodeLoader:
    ...
    def _load_metadata(self) -> None:
        ...
        # Detect the LeRobot dataset codebase version. v3.0 packs many episodes
        # per data/video file and stores episode/task metadata as parquet; v2.x
        # keeps one parquet/video per episode and JSONL metadata. The branches
        # below normalize both into the same in-memory structures
        # (``episodes_metadata`` list-of-dicts + ``tasks_map`` int->str).
        ...
        # Index episode records by their episode_index. v3.0 rows are sorted on
        # load, but file lookups always go through this map so positional drift
        # (e.g. dropped/non-contiguous episodes) can never mis-address a file.
    ...
    def _parse_major_version(codebase_version: str) -> int:
        ...
        Defensive against missing/odd values so an unparseable version falls
        back to the legacy (v2.x) reader rather than crashing at import time.
        """
    ...
    def load_episode(
        ...
        Args:
            ...
            needed_video_indices: If provided, only these (episode-local) video frame
                indices are decoded; the rest of the ``video.*`` column is left as
                ``None``. The caller (``get_shard``) guarantees it only ever reads the
                populated positions, so this decodes ~``episode_sampling_rate`` of the
                frames instead of the whole episode (the expensive HEVC decode is the
                dataloader bottleneck). ``None`` decodes the whole episode (default).
