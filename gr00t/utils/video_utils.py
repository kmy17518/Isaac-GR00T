...
class VideoReaderPool:
    """Per-process LRU cache of persistent video decoders, keyed by file path.

    LeRobot v3.0 concatenates many episodes into a single mp4 per camera, so a
    naive reader rebuilds a decoder on *every* episode access — and for
    torchcodec's default (exact) seek mode that means re-scanning the whole
    concatenated file to build a frame index each time. Reusing one decoder
    across all episodes that live in the same file removes that per-episode
    rebuild while returning byte-identical frames (random access on a decoder is
    stateless w.r.t. the returned pixels).

    Only torchcodec and decord expose a reusable decoder object; ffmpeg/opencv
    spawn a process / open-and-release per call, so for those backends this
    transparently falls back to the stateless :func:`get_frames_by_indices`.

    The pool is intended to be owned by a single ``LeRobotEpisodeLoader`` and is
    therefore confined to one process. With PyTorch ``DataLoader`` workers each
    worker forks and lazily populates its own pool, so cached decoders are never
    shared across processes (which keeps them fork-safe).
    """
    ...
    def get_frames_by_indices(self, video_path: str, indices: list[int] | np.ndarray) -> np.ndarray:
        """Decode frames at ``indices`` reusing a cached decoder when possible.

        Matches :func:`get_frames_by_indices` exactly (same backend dispatch,
        same returned array); the only difference is decoder reuse.
        """
        ...
        # No reusable decoder for ffmpeg/opencv → identical stateless path.
    ...
    def _reader(self, video_path: str, backend: str):
        ...
        while len(self._readers) > self.max_size:
            self._readers.popitem(last=False)  # evict least-recently-used decoder
        return reader
