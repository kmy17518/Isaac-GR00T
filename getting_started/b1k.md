...
- **B300, `sm_103`)** — needs a newer NVRTC. Precompiled kernels are fine (`sm_100` SASS runs on `sm_103`), but torch's bundled CUDA 12.8 NVRTC predates `sm_103`, so every kernel PyTorch compiles *at runtime* (the "jiterator" ops, e.g. `torch.prod` on int64 in Qwen3-VL's `rot_pos_emb`) dies on the very first training step — and again in `serve_b1k.py` — with:
  ...
  The tracked `activate_b300.sh` prepends the wheel's `lib/` dir to `LD_LIBRARY_PATH` (which wins over the RUNPATH torch uses to find `libnvrtc.so.12`), prints `B300 environment configured: NVRTC 12.9.x ...` on success, and is idempotent. A missing or incompatible wheel returns a nonzero status with the install command. The equivalent library-path setting is:
...
- **`torch.compile` on B300 needs a CUDA 13 build of PyTorch — `scripts/deployment/b300/install_cu130_venv.sh` builds it.** The pinned torch 2.7.1+cu128 ships Triton 3.3.1, whose LLVM has no `sm_103` target (`LLVM ERROR: Cannot select: intrinsic %llvm.nvvm.shfl.sync.bfly.i32`), and `sm_100a` binaries are architecture-locked (`no kernel image is available for execution on the device`), so `--compile-blocks` cannot run in the default environment. The CUDA 13 build can compile the kernels, but multi-GPU DeepSpeed has the correctness restrictions in [Training throughput knobs](#training-throughput-knobs); successful compilation alone does not establish safe training. Because the aarch64 `flash-attn` and `torchcodec` wheels in `scripts/deployment/dgpu/wheels/` are built against torch 2.7.1, the script makes a *second* venv rather than editing `uv.lock` (the default `.venv` keeps working for eager training and serving):
  ...
  The script is re-runnable (each phase leaves a marker in `$WORK`, default `$VENV-build`; delete one to redo that phase) and fixes the dangling `.so` symlinks that unpacked `-dev` packages leave behind. Afterwards `source /path/to/venv-cu130/bin/activate`; `activate_b300.sh` is not needed there (CUDA 13's NVRTC knows `sm_103`). Start eager and validate a supported compile setting before enabling it. The checkpoint format is unchanged. Other GPU/framework combinations still require workload-specific gradient validation.
...
The conversion carries an existing `meta/tasks.jsonl` over verbatim (task ids *and* natural-language descriptions); `episodes.jsonl` uses the same task strings as `tasks.jsonl`, as LeRobot v2.1 expects. A partial download without that file produces a legacy task table instead. The modality deployment step below safely enriches matching legacy entries before training.
...
The per-task partial download does not include `meta/tasks.jsonl`, the table both `--prompt-source` options read (see [Language prompt](#language-prompt)). For v3.0, deployment installs matching rows from `examples/b1k/tasks.jsonl` after validating IDs and task names against `meta/tasks.parquet`. For an already converted v2.1 dataset, it enriches a legacy table only when each ID and existing task string matches the reference. It preserves the original `task` strings used by `episodes.jsonl`, adds `task_name` and `task_description`, and points the deployed description annotation at the new field. Duplicate IDs or mismatched text fail before replacing the table. `--dry-run` reports planned changes without writing.
...
**Production safety:** eager is the default. Multi-GPU DeepSpeed accepts only `--compile-blocks dit` (diffusion transformer) or `--compile-blocks vlsa` (vision-language self-attention) individually. Compiling both groups together reproduced corrupted gradients at global batch 4096 on four B300 GPUs with Torch 2.10/CUDA 13 and changing multi-task sequence lengths; finite loss did not reveal the failure. All-block compilation also failed, and disabling communication overlap did not repair it. Unsupported combinations now fail before model loading. Other compile combinations have not passed this production check. This is a conservative safety restriction, not a fix to the underlying compiler/runtime defect. Single-GPU and DDP compilation still require validation for their own workload.

The timing measurements below are historical single-task results, not evidence of multi-task gradient correctness. What a step costs, measured on 4× B300 at 1024 samples per GPU (global batch 4096) on the `turning_on_radio` demos, and the knobs that changed it. Everything marked *exact* produces bit-identical tensors to the stock code path and is verified by tests (`tests/gr00t/model/test_qwen3_vl_fast_positions.py`, `tests/gr00t/model/test_gr00t_processor.py::TestPixelValuesDtype`).
...
- `--compile-blocks dit` or `--compile-blocks vlsa` (+ `--compile-mode`; **not** bit-identical). Wraps selected block forwards without changing parameter names or checkpoint layout. These single-group choices passed the production gradient replay described above; the combination did not. Compilation is training-only and serving remains eager. `vlsa` requires `--compile-persistent-reductions False` on this Blackwell environment. Use an Inductor/Triton supporting the GPU—the default Torch 2.7.1 environment cannot compile for `sm_103`. Historical all-block speed measurements do not apply to the restricted supported configuration; measure it separately and compare gradients with eager.
- **Lossless re-encoded RGB video view** (`scripts/b1k/make_rgb_video_view.py`; *exact*). Video decoding is most of a worker's CPU, for two reasons. First, each shard samples *strided* steps (every ~10th frame of an episode), and with inter-frame coding (the source has a GOP of 8) the decoder must reconstruct every frame in the span to output the ones used — measured single-threaded on the stride-10 pattern, 10.4 ms CPU per used frame for the 720×720 camera and 5.6 ms for each 480×480 one, i.e. ~22 ms per sample, ~10 decoded frames per frame used. Second, the 720×720 stream is decoded at full resolution although the first thing the image pipeline does with every frame is deterministic — `LetterBoxPad` (a no-op on square frames) then `SmallestMaxSize(256, INTER_AREA)` — and only then come the random crop, second resize and colour jitter. The script applies exactly that head once, offline, and stores the 256×256 frames **losslessly in RGB** (`libx264rgb -qp 0`, 4:4:4) with a **GOP of 10 and CAVLC**, so a strided read decodes ~1 frame per frame used and each frame is cheap: 1.8 + 2×1.6 ≈ 5 ms CPU per sample in isolation, ~17 ms inside a fully loaded worker (vs ~70 for a long-GOP lossless file); a worker's total drops from ~80 to ~28 ms CPU per sample. It writes a *view* of the dataset — a new root whose `data/`, `meta/` (including the stats caches), depth streams and untouched chunks are symlinks and only the selected tasks' RGB files are re-encoded, keeping the `.mp4` path template, frame counts and fps (~40 GB per task). Because the stored frames are bit-identical to what the online head produced and the head is the identity on them, every downstream random stage sees identical inputs. Each file is verified bitwise on sampled frames against the source pipeline and recorded in `RGB_VIEW_MANIFEST.json`; a view is only exact for its `--shortest-edge` (256 for N1.7). Beware long-GOP lossless files (x264's default GOP 250 + CABAC): for strided reads they decode *slower* than the source (63 ms per sample measured). Serving is unaffected (live frames take the full pipeline).
  ...
  Source and view roots must be independent, non-nested directories. Expanding the selected tasks safely materializes only the necessary view directories; source videos remain unchanged. Reuse validates source/output hashes and the transformation configuration, including encoder settings and library versions. A legacy manifest, changed source, or incompatible configuration requires a **new `--view-root`**; the script rejects it rather than mixing outputs. Hash validation reads all previously processed source and output videos, so checking a large existing view can take time. Keep a source backup: this fix does not restore videos overwritten by older versions.
...
**Production training command (eager by default)** — 4× B300 (284 GB each), 130-core CPU budget, 900 GiB host RAM limit; global batch 4096 uses roughly 205 GB per GPU with DeepSpeed ZeRO-2. Point `$DATASET_PATH` at the raw dataset or a compatible video view. `$OUTPUT_DIR/$EXP_NAME/checkpoint-<step>/` is where checkpoints land:
...
To train all tasks, omit `--task-names`. Compilation is opt-in: add `--compile-blocks dit` or `--compile-blocks vlsa --compile-persistent-reductions False` only after running the production gradient check below with those settings. The earlier **1.9 s/step** figure used all-block compilation and a single-task GOP-10 view; it is not a timing or correctness claim for this eager command. Watch data-loading waits and host memory when tuning workers.

#### Validate production gradient behavior

Use a fresh disposable output directory and the **same** model, dataset, batch size, precision, distributed backend, worker settings, and compile flags as the intended run. The validation runner keeps the production `--max-steps` learning-rate schedule but stops after a bounded number of optimizer steps. It disables remote reporting/uploads, refuses resume or skipped weight loading, requires precomputed normalization caches, checks unfiltered losses and pre-clipping gradient norms, and requires changing sequence lengths unless `--allow-fixed-length` is supplied.

```bash
export OMP_NUM_THREADS=4 GR00T_FFMPEG_THREADS=4 TOKENIZERS_PARALLELISM=false
export CUDA_VISIBLE_DEVICES=0,1,2,3 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
torchrun --nproc_per_node=4 --master_port=29531 scripts/b1k/validate_training_gradients.py \
    --validation-dir /path/to/fresh-gradient-check --validation-steps 8 \
    --max-observed-grad-norm 5 \
    --base-model-path nvidia/GR00T-N1.7-3B --dataset-path "$DATASET_PATH" \
    --embodiment-tag NEW_EMBODIMENT --modality-config-path examples/b1k/r1pro.py \
    --num-gpus 4 --global-batch-size 4096 --max-steps 150000 \
    --dataloader-num-workers 12 --dataloader-prefetch-factor 1 --decode-only-used-frames \
    --backbone-attn-implementation gr00t_fast
```

Each rank writes `validation-rank<N>.jsonl` and a summary in the validation directory. The default norm threshold is a regression check for this pretrained B1K configuration, not a universal healthy-gradient bound; adjust it only against a measured eager reference. `--save-validation-checkpoint` also exercises checkpoint saving at the final validation step. Use an external job timeout and monitor host/GPU memory: this command allocates real production batches. A short passing run does not establish convergence, compiled/eager gradient equivalence, or correctness at untested shapes. Repeat after changes to the model, data, hardware, framework versions, or compiler options.
...
- **Latest full checkpoint in the current tree** — `hf_resume_checkpoint_uploader.py`. Whenever a newer checkpoint is complete for *resume* (`trainer_state.json` at that step, `latest` → `global_step<step>`, model state and optimizer shards and RNG states for all ranks, `scheduler.pt`, weights, directory unchanged for 2 min), it uploads the whole directory to `<exp>/resume/checkpoint-<step>/` and removes the previous checkpoint folders from the current tree in the same commit, then verifies the uploaded files.

  **Historical objects are retained and continue consuming storage. Automatic irreversible deletion is disabled.** A current-tree scan cannot protect against the evaluation uploader or another writer publishing a reference between the scan and deletion. The monitor preserves replaced-object candidates in `<status-file>.journal/` across failures and restarts. Those records are for offline review, **not a safe deletion list**: collection requires stopping all publishers and checking every branch/tag. Preserve the journal alongside `resume-status.json`. If checkpoint rotation removes an uncommitted pending upload, recovery first checks whether its commit actually succeeded. Otherwise, a newer complete checkpoint may supersede it only after a successor journal is durable; the old candidates remain recorded, and superseded steps never count as uploaded. Without a complete successor, the pending transaction is retained. The current tree contains one full resumable checkpoint, but repository storage is no longer bounded to that checkpoint plus evaluation snapshots; budget quota accordingly.
...
Both scripts need `HF_TOKEN` in the environment (write access to the repo) and poll every 60 s; they exit once the required checkpoints are verified. When upgrading the resume uploader, preserve its journal. Legacy status or remote folder names alone do not prove completeness: retain the latest local checkpoint for verified republication or wait for a newer complete checkpoint. An older local checkpoint never replaces a newer remote one. Lost cleanup candidates from the old implementation cannot be recovered automatically.
